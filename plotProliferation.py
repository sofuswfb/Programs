# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 01:05:37 2024
...
def plot_barchart_for_time(data, time_point, value_column, error_column, title, ttest_results, samples):
    ...
    ymin = 0
    ymax = df_time[value_column].max() * 1.7
    ...
    for (sample1, sample2), p_value in ttest_results.items():
        if p_value < 0.05:
            ...
            add_significance_bar(ax, x1, x2, y, 0.08 * ymax, p_value, level)
            level += 1  # Increment level to stack bars
